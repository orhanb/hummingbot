...
from hummingbot.connector.exchange.btcturk import btcturk_utils as utils


class BtcturkUtilTestCases(unittest.TestCase):
    @classmethod
    ...
    @patch("hummingbot.connector.exchange.btcturk.btcturk_utils.get_tracking_nonce")
    def test_client_order_id_generation(self, nonce_mock):
        ...
        expected_id = (
            f"{CONSTANTS.HBOT_ORDER_ID_PREFIX}-{'B'}"
        ...
        expected_id = (
            f"{CONSTANTS.HBOT_ORDER_ID_PREFIX}-{'S'}"
            f"{self.base_asset[0]}{self.base_asset[-1]}{self.quote_asset[0]}{self.quote_asset[-1]}"
            f"{client_instance_id}{nonce}"
        )
        self.assertEqual(expected_id, client_order_id)
